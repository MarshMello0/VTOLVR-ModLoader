...
namespace DarkRift.Client.Unity
{
	...
	public sealed class UnityClient : MonoBehaviour
	{
        ...
        public void Connect(IPAddress ip, int port, IPVersion ipVersion)
        {
            ...
            if (Connected)
                Console.Log("Connected to " + ip + " on port " + port + " using " + ipVersion + ".");
            else
                Console.Log("Connection failed to " + ip + " on port " + port + " using " + ipVersion + ".");
        }
        ...
        void Client_MessageReceived(object sender, MessageReceivedEventArgs e)
        {
            ...
            {
                if (sniffData)
                    Console.Log("Message Received");      //TODO more information!
        ...
        void Client_Disconnected(object sender, DisconnectedEventArgs e)
        {
            ...
            if (invokeFromDispatcher)
            {
                if (!e.LocalDisconnect)
                    Console.Log("Disconnected from server, error: " + e.Error);
