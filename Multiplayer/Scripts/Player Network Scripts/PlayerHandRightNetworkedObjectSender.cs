...
{
    public class PlayerHandRightNetworkedObjectSender : Sender
    {
        ...
        private void Update()
        {
            ...
            if (Vector3.Distance(lastPosition, transform.position) > moveDistance)
            {
                using (DarkRiftWriter writer = DarkRiftWriter.Create())
                {
                    ...
                    using (Message message = Message.Create((ushort)Tags.PlayerHandRight_Movement, writer))
            ...
            if (lastRotation != transform.rotation)
            {
                using (DarkRiftWriter writer = DarkRiftWriter.Create())
                {
                    ...
                    using (Message message = Message.Create((ushort)Tags.PlayerHandRight_Rotation, writer))
                        client.SendMessage(message, SendMode.Unreliable);
                }
