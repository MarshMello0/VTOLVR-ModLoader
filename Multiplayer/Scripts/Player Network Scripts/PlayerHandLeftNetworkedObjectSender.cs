...
{
    public class PlayerHandLeftNetworkedObjectSender : Sender
    {
        ...
        private void Update()
        {
            ...
            if (Vector3.Distance(lastPosition, transform.position) > moveDistance)
            {
                using (DarkRiftWriter writer = DarkRiftWriter.Create())
                {
                    ...
                    using (Message message = Message.Create((ushort)Tags.PlayerHandLeft_Movement, writer))
            ...
            if (lastRotation != transform.rotation)
            {
                using (DarkRiftWriter writer = DarkRiftWriter.Create())
                {
                    ...
                    using (Message message = Message.Create((ushort)Tags.PlayerHandLeft_Rotation, writer))
                        client.SendMessage(message, SendMode.Unreliable);
                }
