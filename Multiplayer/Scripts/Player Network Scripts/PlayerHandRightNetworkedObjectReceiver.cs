...
{
    public class PlayerHandRightNetworkedObjectReceiver : Receiver
    {
        ...
        private void MessageReceived(object sender, MessageReceivedEventArgs e)
        {
            ...
            {
                if (message.Tag == (ushort)Tags.PlayerHandRight_Movement)
                {
                ...
                else if (message.Tag == (ushort)Tags.PlayerHandRight_Rotation)
                {
                    using (DarkRiftReader reader = message.GetReader())
                    {
                        ushort id = reader.ReadUInt16();
