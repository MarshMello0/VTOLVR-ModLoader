...
{
    public class BasicVehicleNetworkedObjectReceiver : Receiver
    {
        ...
        private void MessageReceived(object sender, MessageReceivedEventArgs e)
        {
            ...
            {
                if (message.Tag == (ushort)Tags.BasicVehicle_Movement)
                {
                ...
                else if (message.Tag == (ushort)Tags.BasicVehicle_Rotation)
                {
                    using (DarkRiftReader reader = message.GetReader())
                    {
                        ushort id = reader.ReadUInt16();
